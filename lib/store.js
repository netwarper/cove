...
class Store {
  ...
  exportLLM(opts = {}) {
    ...
    if (mode === 'perNote') {
      const files = [];
      const seen = {};
      const idxLines = ['# ' + title, '', '> Exported from Daymark on ' + now + ' · ' + collected.length +
    ...
    const head = ['# ' + title, '',
      '> Exported from Daymark on ' + now + ' · ' + collected.length + ' note' + (collected.length === 1 ? '' : 's') + '.',
...
function noteToLLMMarkdown(note, wsName) {
  ...
  if (carry) L.push('### Carryover Notes', '', carry, '');
