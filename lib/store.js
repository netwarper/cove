...
class Store {
  ...
  listNotes(wsId, opts = {}) {
    this._wsMeta(wsId);
    let notes = this._listNoteFiles(wsId).map((f) => this._readEnc(path.join(this._notesDir(wsId), f)));
    const archived = opts.archived || 'active'; // 'active' | 'archived' | 'all'
    if (archived === 'active') notes = notes.filter((n) => !n.archived);
    else if (archived === 'archived') notes = notes.filter((n) => n.archived);
  ...
  /**
   * Create a new note. Incomplete todos + carryover copy from the most recent
   * note (meeting notes do not). A template — passed explicitly or set as the
   * workspace default — then seeds the Meeting Notes section, and fills
   * To-Do / Carryover only when carry-forward left them empty. Finally, due
   * reminders are injected as todos.
   */
  ...
  currentNote(wsId) {
    ...
    if (!note) {
      ...
      const tpl = this._resolveTemplate(this._wsMeta(wsId).defaultTemplateId);
      if (tpl) {
        note.meetingNotes = tpl.meetingNotes || '';
        if ((tpl.defaultTodos || []).length) note.todos = tpl.defaultTodos.map((text) => ({ id: c.randomId(6), text, done: false, doneAt: null, due: null, sourceReminderId: null }));
        if (tpl.defaultCarryover) note.carryover = tpl.defaultCarryover;
      }
      this._injectDueReminders(wsId, note, new Date());
      this._writeEnc(this._notePath(wsId, note.id), note);
      this._indexUpsert(note);
  ...
  saveNote(noteId, patch) {
    ...
    if (!loc) throw httpError(404, 'note not found');
    const note = loc.note;
    // optimistic-concurrency guard for cloud-sync / multi-tab safety
    if (patch.baseUpdatedAt && note.updatedAt && patch.baseUpdatedAt !== note.updatedAt) {
      throw httpError(409, 'note changed elsewhere', { current: note });
    }
    this._snapshotVersion(loc.wsId, note); // history (coalesced)
    const allowed = ['title', 'customTitle', 'todos', 'carryover', 'meetingNotes', 'favorite', 'freeform', 'tags', 'pinned', 'archived'];
  ...
  forkNote(noteId, patch) {
    ...
    if (!loc) throw httpError(404, 'note not found');
    const note = this._blankNote(loc.wsId, { customTitle: (patch.customTitle || loc.note.customTitle || 'Note') + ' (conflict copy)' });
    ['todos', 'carryover', 'meetingNotes', 'freeform', 'tags'].forEach((k) => { if (patch[k] !== undefined) note[k] = patch[k]; });
    if (Array.isArray(note.todos)) note.todos = normalizeTodos(note.todos);
  ...
  listTrash() {
    ...
    for (const w of this._index().workspaces) {
      for (const f of this._listTrashFiles(w.id)) {
        ...
        out.push(Object.assign(noteSummary(n), { workspaceName: w.name, deletedAt: n.deletedAt }));
      }
    }
  ...
  copyNote(noteId, targetWsId) {
    ...
    this._wsMeta(dest);
    const src = loc.note;
    const copy = this._blankNote(dest, { customTitle: (src.customTitle ? src.customTitle + ' (copy)' : 'Copy') });
    copy.tags = (src.tags || []).slice();
    copy.carryover = src.carryover || '';
    copy.meetingNotes = src.meetingNotes || '';
    copy.freeform = JSON.parse(JSON.stringify(src.freeform || []));
  ...
  restoreVersion(noteId, ts) {
    ...
    const note = loc.note;
    ['title', 'customTitle', 'todos', 'carryover', 'meetingNotes', 'tags', 'freeform'].forEach((k) => { if (v[k] !== undefined) note[k] = v[k]; });
    note.updatedAt = new Date().toISOString();
    this._writeEnc(loc.path, note);
    this._indexUpsert(note);
    return note;
