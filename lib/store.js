...
class Store {
  ...
  _writeEnc(file, obj) {
    this._writeBytesAtomic(file, c.encryptJSON(this.key, obj));
  }

  // Write bytes atomically: a full temp file then rename, so a crash or a full
  // disk (ENOSPC) mid-write can never leave a partial/corrupt file in place. On
  // failure the temp is cleaned up rather than left behind next to the real file.
  _writeBytesAtomic(file, buf) {
    ...
    try {
      ...
      try { fs.rmSync(tmp, { force: true }); } catch (_e) { /* best effort */ }
      throw e;
    }
  }
  ...
  // Configured IANA timezone (e.g. "America/New_York"), or null to use the
  // server's local clock. Drives "today", daily-note dates, and reminder firing
  // so they follow the user's timezone even when the server runs elsewhere.
  _tz() { const tz = (this._index().settings || {}).timezone; return tz || null; }
  _todayISO() { return tasks.todayISOInTz(this._tz()); }
  saveSettings(patch) {
    const idx = this._index();
    ...
    // Reject an unusable timezone rather than storing it (empty string clears it,
    // reverting to the server's local clock).
    if (patch.timezone !== undefined && patch.timezone !== '' && !tasks.isValidTimezone(patch.timezone)) {
  ...
  latestDailyNote(wsId) {
    // Use the index (which carries kind + createdAt per live note, and drops
    // trashed notes) to pick the newest non-scratch note, then decrypt just that
    // one file — instead of reading and decrypting every note in the workspace.
    // Candidates are ordered newest-first and we return the first that's readable,
    // so any index/file drift falls back gracefully.
    const notes = this._readIndex().notes || {};
    const candidates = Object.keys(notes)
      .filter((id) => notes[id].wsId === wsId && notes[id].kind !== 'scratch')
      .sort((a, b) => (notes[a].createdAt < notes[b].createdAt ? 1 : notes[a].createdAt > notes[b].createdAt ? -1 : 0));
    for (const id of candidates) {
      const p = this._notePath(wsId, id);
      if (fs.existsSync(p)) { const n = this._readEncSafe(p); if (n) return n; }
    }
    return null;
  }
  ...
  _blankNote(wsId, opts = {}) {
    ...
    return {
      id: c.randomId(8),
      workspaceId: wsId,
      title: this._todayISO(), // daily-note date in the user's configured timezone
  ...
  processInbox(opts = {}) {
    ...
    for (const text of items) store.tasks.push(this._blankTask(wsId, { text: text, sourceInbox: true, due: this._todayISO() }));
  ...
  // Enforce the configured cap on stored completed tasks (per workspace), keeping
  // the most recent ones and dropping the oldest. `completedKeep` = 0 (default)
  // means unlimited. Bounds how much completed history accumulates (and gets
  // scanned) over years; pruning is lazy — applied when a task is completed.
  _capCompleted(store) {
    const cap = parseInt((this._index().settings || {}).completedKeep, 10);
    if (!cap || cap <= 0) return store; // unlimited
    const completed = store.tasks.filter((t) => t.done && t.completedAt);
    if (completed.length <= cap) return store;
    completed.sort((a, b) => (a.completedAt < b.completedAt ? 1 : a.completedAt > b.completedAt ? -1 : 0));
    const dropIds = new Set(completed.slice(cap).map((t) => t.id));
    store.tasks = store.tasks.filter((t) => !dropIds.has(t.id));
    return store;
  }
  ...
  _blankTask(wsId, data = {}) {
    ...
    return {
      ...
      // Additional workspaces this task is shared into. The task lives once (in
      // workspaceId, its "home"); shared workspaces show the same record, so
      // completing it — and its completed date — reflect in all of them.
      sharedWith: Array.isArray(data.sharedWith) ? data.sharedWith.slice() : [],
      createdAt: now,
      updatedAt: now,
  ...
  listTasks(wsId) {
    this._wsMeta(wsId);
    const own = this._readTasks(wsId).tasks;
    // Also surface tasks homed in OTHER workspaces but shared into this one.
    const shared = [];
    for (const w of this._index().workspaces) {
      if (w.id === wsId) continue;
      for (const t of this._readTasks(w.id).tasks) {
        if (Array.isArray(t.sharedWith) && t.sharedWith.includes(wsId)) shared.push(t);
      }
    }
    return shared.length ? own.concat(shared) : own;
  }

  /** Set the extra workspaces a task is shared into (replaces the whole set).
   *  The task keeps its single home record — completion reflects everywhere. */
  shareTask(taskId, workspaceIds, opts = {}) {
    const loc = this._locateTask(taskId, opts.workspaceId);
    if (!loc) throw httpError(404, 'task not found');
    const valid = new Set(this._index().workspaces.map((w) => w.id));
    const set = Array.from(new Set((workspaceIds || [])
      .filter((id) => valid.has(id) && id !== loc.wsId)));
    loc.task.sharedWith = set;
    loc.task.updatedAt = new Date().toISOString();
    this._writeTasks(loc.wsId, loc.store);
    return { workspaceId: loc.wsId, task: loc.task, tasks: loc.store.tasks };
  }
  ...
  addTask(wsId, data) {
    ...
    if (!t.due) t.due = this._todayISO(); // default to today (recurring tasks also need an anchor date)
    store.tasks.push(t);
    this._writeTasks(wsId, store);
  ...
  _locateTask(taskId, wsHint) {
    // Try the hint first (fast path), then fall back to every workspace. A shared
    // task's home may differ from the workspace it was actioned in, so the hint is
    // an optimization, never a requirement.
    const all = this._index().workspaces;
    const order = wsHint ? [{ id: wsHint }].concat(all.filter((w) => w.id !== wsHint)) : all;
  ...
  updateTask(taskId, patch) {
    ...
    if (patch.recurrence !== undefined) { t.recurrence = tasks.normalizeRecurrence(patch.recurrence); if (t.recurrence && !t.due) t.due = this._todayISO(); }
  ...
  completeTask(taskId, opts = {}) {
    ...
    if (t.recurrence && !t.done) {
      ...
      const nd = tasks.nextDue(t.due || this._todayISO(), t.recurrence);
  ...
  moveTask(taskId, destWsId, opts = {}) {
    ...
    const moved = Object.assign({}, loc.task, {
      ...
      // A task can't be shared into its own home; drop the new home from the set.
      sharedWith: (loc.task.sharedWith || []).filter((id) => id !== destWsId),
  ...
  globalTasks() {
    const out = [];
    const wsById = {};
    for (const w of this._index().workspaces) wsById[w.id] = w.name;
    for (const w of this._index().workspaces) {
      for (const t of this._readTasks(w.id).tasks) {
        if (t.done) continue;
        out.push(Object.assign({}, t, { workspaceName: w.name, sharedNames: this._sharedNames(t, wsById) }));
      }
    }
    return out;
  }

  // Resolve a task's sharedWith ids to {id,name} of workspaces that still exist,
  // so the client can label a task with every workspace it's linked into.
  _sharedNames(t, wsById) {
    return (Array.isArray(t.sharedWith) ? t.sharedWith : [])
      .map((id) => ({ id, name: wsById[id] }))
      .filter((x) => x.name);
  }
  ...
  dueTaskNotifications() {
    ...
    const hhmm = tasks.hhmmInTz(this._tz());
    const out = [];
    for (const w of this._index().workspaces) {
      const store = this._readTasks(w.id);
      let changed = false;
  ...
  migrateTasks() {
    ...
    for (const w of idx.workspaces) {
      ...
      for (const r of (meta && meta.reminders) || []) {
        ...
        t.due = (r.cadence && r.cadence.type === 'once') ? (r.cadence.dueDate || this._todayISO())
          : (r.startDate && r.startDate > this._todayISO() ? r.startDate : this._todayISO());
        store.tasks.push(t);
      }
  ...
  addAttachment(noteId, { name, mime, dataB64 }) {
    ...
    this._writeBytesAtomic(this._attPath(loc.wsId, attId), c.encrypt(this.key, raw));
    const meta = {
      id: attId,
