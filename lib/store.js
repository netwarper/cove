...
class Store {
  ...
  createNote(wsId, opts = {}) {
    ...
    if (latest) {
      note.todos = latest.todos
        .filter((t) => !t.done)
        .map((t) => ({ id: c.randomId(6), text: t.text, done: false, doneAt: null, due: t.due || null, sourceReminderId: t.sourceReminderId || null }));
  ...
  saveNote(noteId, patch) {
    ...
    const allowed = ['title', 'customTitle', 'todos', 'carryover', 'meetingNotes', 'favorite', 'freeform', 'tags'];
  ...
  search(query) {
    ...
    if (!q) return [];
    // support a "tag:foo" filter fragment
    let tagFilter = null;
    ...
    for (const w of this._index().workspaces) {
      for (const f of this._listNoteFiles(w.id)) {
        const n = this._readEnc(path.join(this._notesDir(w.id), f));
        if (tagFilter && !(n.tags || []).some((t) => t.toLowerCase() === tagFilter)) continue;
        const hay = [
          displayTitle(n),
          (n.tags || []).join(' '),
        ...
        if (q === '' || at >= 0) {
          results.push({
            ...
            snippet: q ? makeSnippet(hay, at, q.length) : stripHtml(n.meetingNotes).slice(0, 120),
          });
        }
      }
    }
...
function dueOccurrenceKey(rem, today, now) {
  ...
  if (cad.type === 'daily') { if (notYetToday(today)) return null; return today; }
  if (cad.type === 'everyNDays' || cad.type === 'weekly') {
    ...
    if (notYetToday(occurDate)) return null;
    return `${start}+${k * n}`;
  }
  if (cad.type === 'monthly') {
