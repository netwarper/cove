...
class Store {
  ...
  processInbox(opts = {}) {
    ...
    let note = this.latestDailyNote(wsId);
    if (!note) note = this.createNote(wsId, {}); // give the to-dos a home
    note.todos = note.todos || [];
    for (const text of items) {
      note.todos.unshift({ id: c.randomId(6), text: String(text).slice(0, 500), done: false, doneAt: null, due: null, sourceReminderId: null, sourceInbox: true });
    }
    note.todos = normalizeTodos(note.todos);
    note.updatedAt = new Date().toISOString(); // housekeeping write — does not bump rev
    this._writeEnc(this._notePath(wsId, note.id), note);
    this._indexUpsert(note);
    consumed.forEach((p) => { try { fs.rmSync(p, { force: true }); } catch (_e) { /* ignore */ } });
    return { added: items.length, workspaceId: wsId, noteId: note.id };
  }
