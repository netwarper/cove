...
class Store {
  ...
  listNotes(wsId, opts = {}) {
    ...
    return notes.map((n) => Object.assign(noteSummary(n), { doneTaskCount: doneByNote[n.id] || 0 }));
  }
  ...
  processInbox(opts = {}) {
    ...
    for (const text of items) store.tasks.push(this._blankTask(wsId, { text: text, sourceInbox: true }));
  ...
  addTask(wsId, data) {
    ...
    if (t.recurrence && !t.due) t.due = tasks.todayISO(); // a recurring task needs an anchor date
    store.tasks.push(t);
    this._writeTasks(wsId, store);
