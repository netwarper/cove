...
const REC_TYPES = ['daily', 'weekdays', 'weekly', 'monthly', 'yearly', 'everyNDays'];
function normalizeRecurrence(r) {
  ...
  // Interval multiplier for weekly/monthly/yearly ("every 2 weeks", "every 3
  // months", "every 2 years"). Stored only when > 1 so existing single-interval
  // objects are unchanged.
  if (r.type === 'weekly' || r.type === 'monthly' || r.type === 'yearly') {
...
function nextDue(due, rec) {
  ...
  if (rec.type === 'daily') next = addDays(due, 1);
    ...
    // Same month/day, n years later; clamp Feb 29 → Feb 28 in a non-leap year.
    const parts = due.split('-').map(Number); const ny = parts[0] + (rec.n || 1);
    const dim = new Date(Date.UTC(ny, parts[1], 0)).getUTCDate();
    next = ny + '-' + pad(parts[1]) + '-' + pad(Math.min(parts[2], dim));
  }
...
function describeRecurrence(rec) {
  ...
  if (rec.type === 'yearly') return rec.n > 1 ? 'every ' + rec.n + ' years' : 'every year';
