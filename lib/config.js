...
function resolve(dataDir, env) {
  ...
  let port = parseInt(env.PORT, 10) || inst.port ||
    (domain ? derivePort(domain) : DEFAULT_PORT);
  return {
    name: inst.name || 'Meeting Notes',
    domain,
    host,
    port,
...
module.exports = {
  DEFAULT_PORT,
  readInstance, writeInstance, normalizeDomain, derivePort, resolve,
