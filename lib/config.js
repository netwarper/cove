...
function resolve(dataDir, env, opts) {
  ...
  // instance.json port, then the built-in default (3000). Naming an instance no
  // longer auto-assigns a high derived port — the default is a predictable 3000,
  // and you choose another with --port / --set-port / PORT. (derivePort stays
  // available for anyone who wants distinct ports per vault, via --set-port.)
  const cliPort = validPort(opts.port);
  let port = cliPort || parseInt(env.PORT, 10) || inst.port || DEFAULT_PORT;
  return {
    name: inst.name || 'Cove',
    domain,
    host,
...
// Best-effort: name the cloud-sync provider a path lives under, so the UI can
// warn that the notes' availability depends on that app staying connected.
// Matches the folder conventions of each provider on macOS/Windows/Linux.
function cloudProvider(p) {
  const s = String(p || '').replace(/\\/g, '/');
  if (/\/Library\/CloudStorage\/GoogleDrive[^/]*\//i.test(s) || /\/Google ?Drive(\/|$)/i.test(s) || /\/GoogleDrive[^/]*(\/|$)/i.test(s)) return 'Google Drive';
  if (/\/Library\/CloudStorage\/Dropbox[^/]*\//i.test(s) || /\/Dropbox(\/|$)/i.test(s)) return 'Dropbox';
  if (/\/Library\/Mobile Documents\//i.test(s) || /\/iCloud ?Drive(\/|$)/i.test(s) || /\/com~apple~CloudDocs(\/|$)/i.test(s)) return 'iCloud Drive';
  if (/\/Library\/CloudStorage\/OneDrive[^/]*\//i.test(s) || /\/OneDrive[^/]*(\/|$)/i.test(s)) return 'OneDrive';
  if (/\/Library\/CloudStorage\/Box[^/]*\//i.test(s) || /\/Box(\/|$)/i.test(s) || /\/Box Sync(\/|$)/i.test(s)) return 'Box';
  return null;
}
...
module.exports = {
  ...
  readDataDirPointer, writeDataDirPointer, dataDirPointerPath, cloudProvider,
};
